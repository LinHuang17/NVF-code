...
"""
import pdb
def batch_eval_sdf_vote(points, eval_func, num_samples=512 * 512 * 512):
    num_pts = points.shape[1]
    sdf = np.zeros(num_pts)
    vote = np.zeros((21, 4, num_pts))

    num_batches = num_pts // num_samples
    for i in range(num_batches):
        sdf[i * num_samples:i * num_samples + num_samples], vote[:, :, i * num_samples:i * num_samples + num_samples] = eval_func(
                                                                        points[:, i * num_samples:i * num_samples + num_samples])
    if num_pts % num_samples:
        sdf[num_batches * num_samples:], vote[:, :, num_batches * num_samples:] = eval_func(points[:, num_batches * num_samples:])

    return sdf, vote

def eval_sdf_vote_grid(coords, eval_func, num_samples=512 * 512 * 512):
    resolution = coords.shape[1:4]
    coords = coords.reshape([3, -1])
    sdf, vote = batch_eval_sdf_vote(coords, eval_func, num_samples=num_samples)
    return sdf, vote

def eval_sdf_vote_grid_frustum(coords, eval_func, num_samples=512 * 512 * 512):
    # coords = coords.reshape([3, -1])
    sdf, vote = batch_eval_sdf_vote(coords, eval_func, num_samples=num_samples)
    return sdf, vote
