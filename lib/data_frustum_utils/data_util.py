...
def load_train_db_annotation(base_path, writer=None, set_name=None):
    if set_name == 'training':
        if writer is not None:
            writer.print_str('Loading FreiHAND training set index ...')
        t = time.time()
...
def load_eval_db_annotation(base_path, writer=None, set_name=None):
...
def read_img_path(idx, base_path, set_name):
    img_rgb_path = os.path.join(base_path, set_name, 'rgb', '%08d.jpg' % idx)
    if not os.path.exists(img_rgb_path):
        img_rgb_path = os.path.join(base_path, set_name, 'rgb2', '%08d.jpg' % idx)
...
def backproject_ortho(uv, scale,  # kind of the predictions
                      focal, ppt):  # kind of the camera calibration
...
def recover_root(uv_root, scale,
    ...
    xyz_root = backproject_ortho(uv_root, scale, focal, ppt)
    return xyz_root